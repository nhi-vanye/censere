...
birth_death_rates <- ggplot(data=demographics, 
              aes(x=ymd_hms(earth_datetime)) ) + 
      geom_line( aes( y = avg_annual_birth_rate ), color="#009E73" ) +
      geom_line( aes( y = avg_annual_death_rate ), color="#000000" ) +
      geom_hline( aes( yintercept=18.5), color="#009E73", linetype="dashed" ) +
      geom_hline( aes( yintercept=7.5), color="#000000", linetype="dashed" ) +
      
      theme(legend.position="none") +
      labs( x = "Date", y="Population", title="Annual Birth and Death Rates per 1000 population (cf. 2016 Global)", caption="https://en.wikipedia.org/wiki/Birth_rate") 
```
...
population_breakdown <- ggplot( data=populations,
    ...
    facet_wrap(vars(year(earth_datetime)) )
```
